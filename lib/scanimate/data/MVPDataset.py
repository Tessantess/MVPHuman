...
def load_trimesh(ply_files, param_files):
    ...
    for f, smpl_file in tqdm(zip(ply_files, param_files)):
        key_name = f.split('/')[-3].split('_')[0]
        tmp_mesh = trimesh.load(f, process=False)

        R = np.loadtxt(os.path.join(os.path.dirname(f), 'R0.txt'))
        tmp_mesh.vertices = np.matmul(tmp_mesh.vertices, R.T)
...
class MVPDataset_scan(Dataset):  # Single subject under data directory!
    def __init__(self, opt, phase='train', smpl=None, smpl_processing=True,
        ...
        for k, v in self.pkl_data.items():
            ...
            scan_dir = os.path.join(self.root, k)
            for f_name, i in v:
                if self.is_train:
                    if os.path.isfile(os.path.join(scan_dir, f_name[:-4] + self.ext)):
                        pkl_data_list.append(os.path.join(smpl_dir, f_name))
                        ply_data_list.append(os.path.join(scan_dir, f_name[:-4] + self.ext))
                        data_ids.append(i)
                    else:
    ...
    def get_subjects_minimal(self):
        ...
        T_pose_mesh = trimesh.load(file, process=False)
    ...
    def collect_data(self, frame_ratio=None):
        ...
        for sub in self.subjects:
            scan_dir = os.path.join(self.root, sub)
            smpl_dir = os.path.join(self.root, 'smpl')

            if self.is_train:
                data[sub] = sorted([f for f in os.listdir(smpl_dir) if '.npz' in f and
                                    os.path.exists(os.path.join(scan_dir, f[:-4] + self.ext))])
    ...
    def get_item(self, f_id, sub_id, subject, pkl_file):
        ...
        if self.smpl_processing:
            ...
            if not self.is_train:
                ...
                body_pose = torch.Tensor(smpl_data['pose'])
            transl = torch.Tensor(smpl_data['trans']) * 0
...
class CapeDataset_scan_color(MVPDataset_scan):  # Single subject under data directory! Vertex color as supervision
    def __init__(self, opt, phase='train', smpl=None,
                 smpl_processing=True, ground_level=(0, -0.563, 0),
                 customized_minimal_ply='',
                 full_test=True, device=torch.device('cuda:0')):
        MVPDataset_scan.__init__(self, opt=opt, phase=phase, smpl=smpl,
    ...
    def get_item(self, f_id, sub_id, subject, pkl_file):
        ...
        if self.phase == 'train':
            ...
            if self.resample_flag:
                ...
                if not ply_key in self.g_sample_dic:
                    samples, normals, colors, face_id = sample_surface_wnormalcolor(mesh, self.opt['num_sample_surf'],
                                                                                    mask)
                    self.g_sample_dic[ply_key] = {'samples': samples, 'normals': normals, 'face_id': face_id,
                                                  'colors': colors}
                else:
            ...
            if not self.is_train:
                res.update({'scan_posed': samples_cape, 'faces': faces_cape,
                            'original_colors': mesh.visual.vertex_colors[:, :3]})
        else:
            res = {
                'frame_name': ply_key,
                'subject': subject,
...
if __name__ == '__main__':
    tmp_dir = '/media/liaotingting/usb2/tmp/example_03375_shortlong'
    sub = '197019'
    smpl_dir = f'/media/liaotingting/usb/Params/{sub}/indoor/Action03/SMPL'
    obj_path = f'/media/liaotingting/usb/Data/{sub}/indoor/Action03/Action03.obj'
    transform_path = f'/media/liaotingting/usb/Params/{sub}/indoor/Action03/FViewPOSE/R0.txt'

    os.makedirs(f'{tmp_dir}/seq', exist_ok=True)
    # os.system(f'cp {smpl_dir}/minimally.obj {tmp_dir}')
    # os.system(f'cp {smpl_dir}/smpl_param.npz {tmp_dir}/seq/Action03.npz')
    # os.system(f'cp {obj_path} {tmp_dir}/seq')
    os.system(f'cp {transform_path} {tmp_dir}/seq/R.txt')
    # os.system('python -m apps.train_scanimate -c ./configs/mvp.yaml')
    # cp results
